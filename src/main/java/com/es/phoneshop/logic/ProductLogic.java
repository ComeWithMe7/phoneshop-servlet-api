    ...
    private static List<Product> sortingByParam(String sortingParameter, List<Product> productList) {
        if (sortingParameter != null) {
            switch (sortingParameter) {
                case "upDescription":
                    return productList.stream().sorted((x, y) -> x.getDescription().compareTo(y.getDescription())).collect(Collectors.toList());
                case "downDescription":
                    return productList.stream().sorted((x, y) -> y.getDescription().compareTo(x.getDescription())).collect(Collectors.toList());
                case "upPrice":
                    return productList.stream().sorted((x, y) -> x.getPrice().compareTo(y.getPrice())).collect(Collectors.toList());
                case "downPrice":
                    return productList.stream().sorted((x, y) -> y.getPrice().compareTo(x.getPrice())).collect(Collectors.toList());
                default:
                    return productList;
            }
        } else {
